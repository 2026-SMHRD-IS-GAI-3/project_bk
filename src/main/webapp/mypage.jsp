...
<%
    ...
    String equippedTitle = loginUser.getGoods();

    if (equippedTitle == null || equippedTitle.trim().isEmpty()) {
        equippedTitle = "";
    }

    String equippedTitleJs = equippedTitle
        .replace("\\", "\\\\")
        .replace("\"", "\\\"")
        .replace("\r", "")
        .replace("\n", " ");
...
<body>
    ...
    <main class="mypage">
      <section class="profile-panel" aria-label="회원 프로필">
        ...
        <div class="profile-content">
          ...
          <div>
            <span class="profile-title" id="equippedTitle" style="<%= equippedTitle.equals("") ? "display:none;" : "display:inline-flex;" %>"><%= equippedTitle %></span>
            <h1 class="profile-name" id="memberName"><%= loginUser.getName() %></h1>
  ...
  <script>
    ...
    const DB_EQUIPPED_TITLE = "<%= equippedTitleJs %>";

    const ITEM_LIST = [
      { ITEM_NUM: 1, ITEM_TYPE: "TITLE", ITEM_ICON: "rookie", ITEM_NAME: "견습기사" },
      { ITEM_NUM: 2, ITEM_TYPE: "TITLE", ITEM_ICON: "sword", ITEM_NAME: "초심의 검" },
      { ITEM_NUM: 3, ITEM_TYPE: "TITLE", ITEM_ICON: "stance", ITEM_NAME: "반석의 검" },
      { ITEM_NUM: 4, ITEM_TYPE: "TITLE", ITEM_ICON: "flame", ITEM_NAME: "일섬" },
    ...
    function renderProfile() {
      ...
      const equippedTitleElement = document.getElementById("equippedTitle");

      if (DB_EQUIPPED_TITLE && DB_EQUIPPED_TITLE.trim() !== "") {
        equippedTitleElement.innerText = DB_EQUIPPED_TITLE;
        equippedTitleElement.style.display = "inline-flex";
      } else {
        equippedTitleElement.innerText = "";
        equippedTitleElement.style.display = "none";
      }

      document.getElementById("memberName").innerText = "<%= loginUser.getName() %>";
    ...
    function renderStats() {
      ...
      document.getElementById("completedStageCount").innerText = `${DB_COMPLETE_COUNT}개`;
      document.getElementById("averageAccuracy").innerText = DB_COMPLETE_COUNT > 0 ? `${DB_AVG_ACCURACY}%` : "-";
    }
